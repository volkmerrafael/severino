...
public class PontoResource extends ResourceCRUD<Ponto> {
	...
	public List<Ponto> listarPontos(String token, Integer ano, Integer mes) {
		...
		TypedQuery<Ponto> queryPonto = this.getEm().createQuery("select p from Ponto p JOIN FETCH p.importacao i where p.usuario = :usuario and Month(p.data) = :mes and Year(p.data) = :ano order by p.data asc", Ponto.class);
